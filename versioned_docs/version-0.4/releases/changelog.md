...
- **Mode-aware denial log levels**: STRICT mode denials now log at ERROR level instead of WARN, making swallowed denials from third-party libraries impossible to miss. AUDIT and PERMISSIVE modes remain at WARN.
- **Always-on denial counters with JMX MBean**: A new `io.jguard:type=DenialCounters` JMX MBean provides real-time per-operation and total denial counts. Zero configuration required — counters are always active and accessible via JConsole, VisualVM, or any JMX client.
- **Full stack traces in error handling**: `handleError()` now passes the exception to the logger, so enforcement errors include full stack traces for easier debugging.
...
- **runWithAgent duplicate policy discovery**: Fixed the `runWithAgent` task to exclude `build/jguard-policy` from the classpath when the project JAR already contains embedded policies, preventing duplicate policy discovery errors.
